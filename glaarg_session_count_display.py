...
def session_count_data(self):
    ...
    self.topwin.title("GLAARG Session Count Report")
    ...
    if self.my_callsign == "NOCALL":
        ...
        tmp_result = []
        sql_by_total_higher_count = "SELECT COUNT(*) FROM glaarg_count WHERE sess_ct > ?"
        ...
        sql_by_total_equal_count = "SELECT COUNT(*) FROM glaarg_count WHERE sess_ct = ?"
        db_cursor.execute(sql_by_total_equal_count,tuple(tmp_result))
        result = db_cursor.fetchall()
        self.ve_total_equal = result[0][0]
        
        sql_by_total_lower_count = "SELECT COUNT(*) FROM glaarg_count WHERE sess_ct < ?"
        ...
        ## Report texts
        text_text = "Statistics for GLAARG accredited VEs as of {}.\n\n".format(self.db_date)
